...
AEMO price lookup:
    Each reading is matched to the 30-minute clock block that contains it.
...
class WholesaleStrategy:
    ...
    def __init__(self, wholesale_adder_cents_per_kwh: Decimal = Decimal("18.0")) -> None:
    ...
    def compute(
        ...
        *,
        ...
        # Build a fast lookup: block_start → rrp (Decimal)
        price_lookup: dict[datetime, Decimal] = {}
        for p in (aemo_prices or []):
            key = block_start_for(p.interval_start)
            price_lookup[key] = Decimal(str(p.rrp_per_mwh))
        ...
        for reading in readings:
            block = block_start_for(reading.ts)
            rrp = price_lookup.get(block)
            if rrp is None:
                continue  # no price data → skip this reading

            kwh_import = Decimal(str(max(reading.grid_w, 0))) * Decimal(str(_INTERVAL_HOURS)) / Decimal("1000")
            kwh_export = Decimal(str(max(-reading.grid_w, 0))) * Decimal(str(_INTERVAL_HOURS)) / Decimal("1000")
        ...
        if import_kwh > 0 or total_import_cents != 0:
            period_breakdown["wholesale"] = PeriodResult(kwh=import_kwh, cents=total_import_cents)
